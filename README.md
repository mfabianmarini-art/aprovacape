...
- **Validação documental → check-list**: o check-list técnico só libera depois que todos os documentos de `DOC_ORDER` são marcados como validados pelo analista.
...
- **Complementação**: com a solicitação em `COMPLEMENTO`, `/requerimentos` abre a lista dos quatro documentos com botão de substituir em cada um (os apontados pelo analista ficam destacados), e só então "Enviar complementação" devolve a solicitação à CAPE. `uploadDocumentoAction` aceita `RASCUNHO` e `COMPLEMENTO` — nos demais status a solicitação está com a CAPE e não recebe arquivo novo.
