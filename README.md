...
- **Mapa do loteamento**: pins posicionados por `posX`/`posY` (%) sobre a planta, coloridos pelo status da solicitação mais recente do lote. Clicar num lote abre o painel dele com **um card por protocolo** — status, tipo de obra, data de abertura e número de movimentações —, e cada card expande no próprio histórico daquele protocolo, com atalho para a tela de análise. Lotes com mais de uma solicitação deixam de ter as movimentações misturadas numa linha do tempo única.
